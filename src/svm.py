...
def train(data_dict, n_train, n_val):
    ...
    print("Support Vector Machine à Noyau Gaussien (RBF Kernel SVM) Classifier")

    model = make_pipeline(
        StandardScaler(),
        PCA(n_components=0.95),
        SVC(kernel="rbf", C=1.0, gamma="scale", random_state=42, verbose=0),
    )
...
def get_decision_function(image):
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            f"Fichier modèle {model_path} non trouvé. "
            "Assurez-vous d'avoir entraîné le modèle avant d'appeler get_decision_function()."
        )
    ...
    image = np.array(image, dtype=np.float32).reshape(1, -1)
    decision_values = model.decision_function(image)[0]
